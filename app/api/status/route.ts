...
export async function GET() {
  ...
  try {
    ...
    const payload = await response.json() as { EndPoint?: string; Data?: { hostname?: string; clients?: number; sv_maxclients?: number; svMaxclients?: number; lastSeen?: string; gametype?: string; mapname?: string; resources?: string[]; players?: Array<{id?:number;name?:string;ping?:number}> } };
    ...
    const playersList=(payload.Data.players||[]).map((player)=>({id:Number(player.id||0),name:String(player.name||'Ukendt spiller').slice(0,80),ping:Number(player.ping||0)})).sort((a,b)=>a.name.localeCompare(b.name,'da'));
